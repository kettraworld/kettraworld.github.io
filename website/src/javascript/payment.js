...
$("#modal-shop").on('show.bs.modal', (e) => {
  $('#shop #email').val(Cookies.get('GOOGLE_EMAIL'));
  $("#shop #id").val(e.relatedTarget.getAttribute('product'))
...
axios.post('http://129.148.43.196:8080/product/coupon', {
  id: $("#shop #id").val(),
  key: $("#shop #coupon").val()
}).then((response) => {
    if (response.status === 200) {
      ...
      message(`Parabéns, você recebeu <b style="color: green;">${response.data.discount}%</b> de desconto na sua compra!`);
    } else if (response.status === 201) {
      message('O código promocional já expirou ou não está disponível para este produto!');
    } else {
      message('Por favor, tente novamente mais tarde. Se o problema continuar, entre em contato pelo <a href="https://discord.gg/d3YgZTfZq5">discord</a>.');
    }
}).catch(() => {
...
axios.post('http://129.148.43.196:8080/payment/create', {
  product: $("#shop #id").val(),
...
axios.get('http://129.148.43.196:8080/product').then((response) => {
  ...
  $('.category').click(function() {
    ...
    if (!Cookies.get('GOOGLE_UID')) {
      return message('Você deve estar logado com o google para poder realizar a compra!');
    }
